...
module.exports = {
    ...
    run: async(client, message, args) => {
      ...
      if(purchase == 'Tea') {
            if(cash < 100) {
                const purchaseError = new MessageEmbed()
                .setDescription("You Don\'t Have Enough Money To Buy A Tea!")
                .setColor("BLUE")
            ...
            db.push(`items_${message.author.id}`, "Tea")

            const purchaseTeaSuccess = new MessageEmbed()
            .setDescription(`Successfuly Bought One Tea For 100${emote.coin}`)
            .setColor("BLACK")

            message.channel.send(purchaseTeaSuccess)
      ...
      if(purchase == 'Cofie') {
            if(cash < 110) {
                const purchaseError = new MessageEmbed()
                .setDescription("You Don\'t Have Enough Money To Buy A Cofie!")
                .setColor("BLUE")

                return message.channel.send(purchaseError)
            }

            db.subtract(`money_${message.author.id}`, 110)
            db.push(`items_${message.author.id}`, "Cofie")

            const purchaseCofieSuccess = new MessageEmbed()
            .setDescription(`Successfuly Bought One Thief Outfit For 110${emote.coin}`)
            .setColor("BLACK")

            message.channel.send(purchaseCofieSuccess)
            ...
            .setDescription(`Successfuly Bought One Gift Box For 1200:coin:`)
            .setColor("BLACK")
